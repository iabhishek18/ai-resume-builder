...
def calculate_keyword_match(resume_text: str, job_description: str) -> float:
    ...
    common_words = {"the", "and", "for", "that", "with", "this", "from", "are", "was", "will", "have", "been"}
...
async def score_resume(resume_text: str, job_description: str) -> ATSResult:
    ...
    response = client.chat.completions.create(
        model="gpt-4",
        messages=[
            {"role": "system", "content": "You are an ATS (Applicant Tracking System) expert. Analyze the resume against the job description."},
